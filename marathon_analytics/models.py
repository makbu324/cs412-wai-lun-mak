...
class Result(models.Model):
    ...
    def get_runners_passed(self):
        ...
        passed = start_before.filter(finish_time_of_day__lt=self.finish_time_of_day)
        return len(passed)
    
    def get_runners_passed_by(self):
        start_after = Result.objects.filter(start_time_of_day__gt=self.start_time_of_day)
        passed_by = start_after.filter(finish_time_of_day__gt=self.finish_time_of_day)
        return len(passed_by) 
    
def load_data():
    '''Function to load data records from CSV file into Django model instances.'''
    filename = 'C:/Users/2023_chicago_results.csv'
    f = open(filename)
